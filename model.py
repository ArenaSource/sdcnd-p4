...
class Image(object):
    ...
    def fit(self, windows=9):

        # Create an output image to draw on and visualize the result
        out_img = np.dstack((np.zeros_like(self.pixels), np.zeros_like(self.pixels), np.zeros_like(self.pixels))) * 255
    ...
    def adjust(self, left_fit, right_fit):
